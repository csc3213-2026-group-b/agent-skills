...
> _caution_: This is not a guide on how to use e.g. a user's email as their WebAuthn ID. Values used for `userID` **MUST NOT** contain personally-identifying information (PII)! A value like `"WA1234567890ABC"` is okay, but a value like `"iamkale@simplewebauthn.dev"` is not!. If you don't want to worry about any of this then consider skipping the rest of this guide and revisiting the general guidance offered in [Packages &gt; @simplewebauthn/server](../../packages/server.md).
